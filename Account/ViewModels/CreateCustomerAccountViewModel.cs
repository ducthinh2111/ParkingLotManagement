...
{
    public class CreateCustomerAccountViewModel : BindableBase, INavigationAware
    {
        ...
        public CreateCustomerAccountViewModel(ICustomersRepository customersRepository, IRegionManager regionManager)
        {
        ...
        private bool isDuplicatedStaff(string ID, string email, string phoneNumber, string userName)
        {
            foreach (var cus in _customersList)
            {
                if (cus.ID == ID)
                {
                    MessageBox.Show("This ID has been used by another customer!", "Data in use", MessageBoxButton.OK, MessageBoxImage.Asterisk);
                    return true;
                }
                if (cus.Email == email)
                {
                    MessageBox.Show("This email has been used by another customer!", "Data in use", MessageBoxButton.OK, MessageBoxImage.Asterisk);
                    return true;
                }
                if (cus.PhoneNumber == phoneNumber)
                {
                    MessageBox.Show("This phone number has been used by another customer!", "Data in use", MessageBoxButton.OK, MessageBoxImage.Asterisk);

                    return true;
                }
                if (cus.Username == userName)
                {
                    MessageBox.Show("This user name has been existed!", "Data in use", MessageBoxButton.OK, MessageBoxImage.Asterisk);
                    return true;
                }
            }
            return false;
